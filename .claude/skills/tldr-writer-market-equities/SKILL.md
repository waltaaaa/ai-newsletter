...
```html
<p><span class="lead-sentence">The S&P/TSX Composite closed at 24,590
on Friday, up 0.8% on the week</span> — the materials sub-index led
with a 3.2% gain as gold reached US$2,280/oz, while the
energy sub-index declined 1.4% on WTI weakness<sup>1</sup>. Financials,
the index's largest sector weight, added 0.6% as the yield curve
steepened<sup>1</sup>. Year-over-year, the TSX is up 12.4%, with the
index trading 4.2% below its 52-week high of 25,670
reached in January<sup>2</sup>. The database tracks 89 mining projects
($14.2 billion) that are precious-metals-linked<sup>3</sup>.</p>
```
...
**AFTER:**
```html
<p><span class="lead-sentence">The S&P 500 closed at 5,280, up
1.1% on the week</span> — information technology and communication
services led gains as the Federal Reserve held rates at 5.33-5.58%<sup>4</sup>.
The index trades 3.8% below its 52-week high, with year-to-date
performance at +8.2%<sup>5</sup>.</p>
```
...
39,450, up 0.9% on the week</span> — industrials and
healthcare components contributed the largest point gains, while energy-weighted names
declined on lower crude prices<sup>6</sup>. The DJIA is up 6.8%
...
8. **Don't emit `<strong>` or `<b>`.** They are banned everywhere in prose output. The lead-in sentence is the only bold text the reader sees, via frontend CSS.
