...
12. **Pipeline data is the ONLY source for prices (hard rule, validator-enforced).** Every `val`, yield, FX rate, and index level you emit MUST be the value carried in `dossier_macro.json` (which is built from `timeseries.json`). Never substitute a number from WebSearch, memory, or a prior edition — the 2026-06-08 edition shipped wheat at $671.75 (actual $583), silver off by 18%, and the prior edition's potash price relabeled as current, all from exactly this failure. `tools/validate_briefing_schema.py` now reconciles every structured print against `timeseries.json` and hard-FAILs the deploy gate on >5% divergence for a fresh edition. If a dossier value looks wrong or stale, mark the item N/A and flag it in your completion report — do not "fix" it from the web.
13. **"On the week" means 7 days.** The previous edition is typically 1–3 weeks old; a move measured against the prior edition's baseline must be labeled "since the last edition (May 19)" or "over the inter-edition period" — never "on the week" or "weekly". Use the dossier's `weekly_pct` (computed from timeseries 7-day windows) for true weekly language.
