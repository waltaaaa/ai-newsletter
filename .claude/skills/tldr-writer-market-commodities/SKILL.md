...
- Per-commodity narratives use the lead-sentence em dash pattern: `<p><span class="lead-sentence">{Name} settled at {price} ({weekly_pct} week-over-week)</span> — {context}.<sup>N</sup></p>`
...
```html
<p><span class="lead-sentence">WTI crude settled at US$67.20/bbl (-6.7%
week-over-week)</span> — OPEC+ confirmed a production increase of 400,000 barrels per
day effective May 1, while the IEA reported global demand growth below prior
estimates<sup>1</sup>. The project database contains 312 energy projects
($87.4 billion), of which 23 have estimated breakeven costs above the
current WTI price — these are concentrated in Alberta oil sands and Saskatchewan
heavy oil<sup>2</sup>. WTI is down 18.2% year-over-year and trades
22% below its 52-week high of US$86.40<sup>1</sup>.</p>
```
...
**AFTER:**
```html
<p><span class="lead-sentence">Western Canadian Select settled at US$54.80/bbl
(-7.2% week-over-week)</span> — the WCS-WTI discount widened to
US$12.40/bbl, up from $10.80 the prior week, reflecting
pipeline capacity constraints and heavy crude oversupply<sup>3</sup>. At the current
WCS price, 8 Alberta oil sands projects ($4.1 billion) in the database
...
**AFTER:**
```html
<p><span class="lead-sentence">Gold settled at US$2,145/oz (+1.2%
week-over-week)</span> — safe-haven demand increased as US real yields declined and central
bank purchases continued<sup>4</sup>. The database tracks 67 precious metals
mining projects ($8.4 billion) across BC, Ontario, and Quebec<sup>2</sup>.
Gold is up 14.8% year-over-year<sup>4</sup>.</p>
```
...
**AFTER:**
```html
<p><span class="lead-sentence">Lumber settled at US$450/mfbm (-12.1%
week-over-week)</span> — the lowest price in 38 weeks as US housing starts declined and
Canadian mills increased production<sup>5</sup>. Lower lumber prices reduce input costs
for the 312 residential construction projects ($54.1 billion) in the
database<sup>2</sup>. However, BC forestry operations face margin compression at prices
below US$500/mfbm<sup>6</sup>.</p>
```
...
down 6.7% and natural gas down 8.4%, while precious
metals advanced as gold gained 1.2% on safe-haven flows<sup>1,4</sup>.
The WCS-WTI discount widened to US$12.40/bbl, increasing the cost
disadvantage for Canadian heavy crude producers<sup>3</sup>. Base metals were mixed:
copper fell 3.1% on Chinese demand concerns while uranium held steady
on nuclear plant construction demand<sup>7,8</sup>. Agricultural commodities firmed,
with wheat up 2.3% and canola up 1.8%<sup>9</sup>.
The project database tracks $312 billion in commodity-linked capital
...
```html
<p><span class="lead-sentence">{Name} settled at {Price with units}
({weekly_pct} week-over-week)</span> — {1-2 sentences: driver of the move +
...
for c in data['commodities']:
    if c.get('commentary') and '<span class="lead-sentence">' not in c['commentary']:
        print(f"FAIL — {c.get('name', 'unknown')} narrative missing em dash lead sentence")

# ── BOLD BAN CHECK ──
if '<strong>' in all_html or '<b>' in all_html:
...
11. **Don't emit `<strong>` or `<b>`.** They are banned everywhere in prose output. The lead-in sentence is the only bold text the reader sees, via frontend CSS. Open every per-commodity narrative with `<span class="lead-sentence">` instead of a bolded name/price.
