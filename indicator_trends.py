...
def compute_indicator_trends(conn):
    """Main entry point: compute trends for all key indicators.

    Reads from SQLite timeseries table.
    ...
    if hasattr(conn, 'execute'):
        # SQLite path
        from db import get_timeseries, get_dashboard_state

        for indicator in KEY_INDICATORS:
            try:
                rows = get_timeseries(conn, indicator, limit=52)
                if not rows:
                    results[indicator] = {"direction": "no_data"}
                    continue

                # Rows are newest-first from get_timeseries; reverse for oldest-first
                values = [(r["date"], r["value"]) for r in reversed(rows)]
        ...
        # Check latest two newsletters for comparison
        try:
            current_nl = get_dashboard_state(conn, "newsletter_latest") or {}
            if current_nl:
                previous_nl = get_dashboard_state(conn, "newsletter_previous") or {}
                if previous_nl:
        ...
        ts_ref = conn.collection("timeseries")
        ...
        try:
            nl_docs = list(conn.collection("newsletters")
