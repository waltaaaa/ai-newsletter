...
def generate_trend_report(sector_data, indicator_data, xref_data, conn=None, db=None):
    ...
    Args:
        ...
        conn: optional sqlite3.Connection to store the report
        db: deprecated; ignored (kept for backward compatibility)
    ...
    if conn:
        try:
            ...
            save_dashboard_state(conn, f"trend_report_{week_key}", report)
            print(f"  [REPORT] Stored as trend_report_{week_key}")
        except Exception as e:
