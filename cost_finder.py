...
def select_projects_needing_cost(conn, max_candidates=MAX_COST_QUERIES_PER_RUN):
    """Select projects that have no dollar value.
    ...
    if hasattr(conn, 'execute'):
        ...
        for data in all_projects:
            ...
            if last_search:
                try:
                    ls = datetime.fromisoformat(str(last_search)[:10])
                    if (now - ls).days < COST_SEARCH_COOLDOWN_DAYS:
                        continue
    ...
    candidates.sort(key=lambda x: x[2], reverse=True)
    return [(doc_id, data) for doc_id, data, _ in candidates[:max_candidates]]
...
async def run_cost_finding(conn):
    ...
    for doc_id, project in candidates:
        ...
        if cost["found"] and cost["value_millions"] and cost["value_millions"] > 0:
            ...
            if use_sqlite:
                try:
                    row = conn.execute(
                        "SELECT evidence, confidence FROM projects WHERE norm_key = ?",
                        (doc_id,),
                    ).fetchone()
                    if row:
                    ...
                    logger.warning(f"[COST] Evidence merge failed: {e}")
            else:
                try:
            ...
            if use_sqlite:
                ...
                try:
                    ...
                    logger.warning(f"[COST] Update failed for {doc_id}: {e}")
            else:
...
def run_cost_search(conn):
    ...
    if loop and loop.is_running():
        ...
        try:
            ...
            return {"found": 0, "not_found": 0, "unfindable": 0, "errors": 0}
        return loop.run_until_complete(run_cost_finding(conn))
    else:
        return asyncio.run(run_cost_finding(conn))
