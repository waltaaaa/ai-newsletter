...
async function renderInteractiveMap(){
  ...
  const statsDefault=[
    ...
    {label:'Wage Growth',value:pick(_tWage&&_tWage.value,m.wageGrowth,_indVal('wageGrowth')),change:pick(computeChange('wageGrowth','national'),(()=>{const v=pick(_tWage&&_tWage.value,m.wageGrowth);return v&&/^[+-]?\d/.test(String(v))&&String(v).includes('%')?String(v):''})()),period:indBasis(_tWage,'','monthly'),freq:'Monthly',source:indSource(_tWage,'Statistics Canada')},
...
async function renderCanadaSub(){
  ...
  const natIndicators=[
    ...
    {label:'Wage Growth',value:pick(natWage&&natWage.value,m.wageGrowth,indVal('wageGrowth')),change:pick(computeChange('wageGrowth','national'),(()=>{const v=pick(natWage&&natWage.value,m.wageGrowth);return v&&/^[+-]?\d/.test(String(v))&&String(v).includes('%')?String(v):''})()),source:indSource(natWage,'Statistics Canada'),metaKey:'wageGrowth',period:indBasis(natWage,'','monthly'),freq:'Monthly'},
