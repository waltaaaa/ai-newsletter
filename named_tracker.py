...
def select_top_projects(conn, max_candidates=MAX_TRACKING_PER_RUN):
    ...
    if hasattr(conn, 'execute'):
        ...
        # Use norm_key as the identifier in SQLite
        for data in all_projects:
            ...
            candidates.append((norm_key, data, value_m, days_since_check))
...
def apply_tracking_results(conn, results):
    """Write tracking results to SQLite.
    ...
    for r in results:
        ...
        doc_id = r["query"]["doc_id"]  # norm_key for SQLite, doc.id for Firestore
        parsed = r.get("parsed")
        if not parsed:
            errors += 1
            continue
            ...
            if parsed.get("status_changed") and parsed.get("status"):
                ...
                if use_sqlite:
                    try:
                        ...
                        history = _json.loads(row["statusHistory"] or "[]") if row else []
                        entry = {
                            "date": today,
                            "status": new_status,
                        ...
                        updates["statusHistory"] = _json.dumps(history, ensure_ascii=False)
                    except Exception:
                        pass
        ...
        if use_sqlite:
            ...
            try:
                ...
                logger.warning(f"[TRACK] Update failed for {doc_id}: {e}")
        else:
            # Legacy Firestore
...
async def run_named_tracking_async(conn, max_queries=MAX_TRACKING_PER_RUN):
    """Run tracking queries for top projects by value."""
    from gemini_engine import run_batch

    candidates = select_top_projects(conn, max_queries)
    if not candidates:
    ...
    summary = apply_tracking_results(conn, results)
    print(f"  [TRACK] {summary['updated']} updates, "
          f"{summary['checked']} checked, {summary['errors']} errors")
    return summary


def run_named_tracking_sync(conn, max_queries=MAX_TRACKING_PER_RUN):
    """Synchronous wrapper."""
    import asyncio
    try:
        ...
        if loop.is_running():
            ...
            return loop.run_until_complete(
                run_named_tracking_async(conn, max_queries))
        else:
            return asyncio.run(run_named_tracking_async(conn, max_queries))
    except RuntimeError:
        return asyncio.run(run_named_tracking_async(conn, max_queries))
