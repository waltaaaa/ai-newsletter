...
async def select_microscope_topic(db, rss_articles, indicator_trends, cross_insights):
    ...
    1. Manual override from Firestore dashboard_state/microscope_override
    ...
    try:
        ...
        if override_doc.exists:
            ...
            if override.get("active") and override.get("topic"):
                ...
                db.collection("dashboard_state").document("microscope_override").update(
                    {"active": False}
                )
    ...
    try:
        ...
        return await _build_topic_context(
            db, topic, description, affected_sectors, affected_provinces, rss_articles
        )
...
async def _build_topic_context(db, topic, description, affected_sectors,
    ...
    history = []
    try:
        history_doc = db.collection("dashboard_state").document("microscope_history").get()
        if history_doc.exists:
            history = history_doc.to_dict().get("topics", [])
...
def store_microscope_history(db, topic, analysis_text):
    ...
    try:
        ...
        db.collection("dashboard_state").document("microscope_history").set(
            {"topics": history}
        )
...
def get_affected_projects(db, topic_context):
    """Query Firestore for projects that may be affected by the microscope topic.
    ...
    try:
        ...
        for sector in affected_sectors[:3]:
            docs = db.collection("projects").where(
                "sector", "==", sector
            ).limit(20).stream()
            for doc in docs:
                d = doc.to_dict()
