...
def select_projects_for_verification(db, max_candidates=MAX_VERIFICATION_PER_RUN):
...
def apply_verification_results(db, results):
    """Write verification results back to Firestore.
    ...
    for r in results:
        ...
        if parsed.get("confirmed"):
            ...
            # Update value if found and project has none
...
async def run_deep_verification_async(db, max_queries=MAX_VERIFICATION_PER_RUN):
    """Run verification queries for single-source projects."""
    from gemini_engine import run_batch

    candidates = select_projects_for_verification(db, max_queries)
    if not candidates:
    ...
    summary = apply_verification_results(db, results)
    print(f"  [VERIFY] {summary['confirmed']} confirmed, "
          f"{summary['unconfirmed']} unconfirmed, {summary['errors']} errors")
    return summary


def run_verification(db, max_queries=MAX_VERIFICATION_PER_RUN):
    """Synchronous wrapper."""
    import asyncio
    try:
        ...
        if loop.is_running():
            ...
            return loop.run_until_complete(
                run_deep_verification_async(db, max_queries))
        else:
            return asyncio.run(run_deep_verification_async(db, max_queries))
    except RuntimeError:
        return asyncio.run(run_deep_verification_async(db, max_queries))
