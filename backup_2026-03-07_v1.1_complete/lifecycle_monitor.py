...
def select_projects_for_monitoring(db, max_candidates=MAX_MONITOR_PER_RUN):
...
async def run_lifecycle_monitoring(db):
    ...
    candidates = select_projects_for_monitoring(db)
    if not candidates:
    ...
    for i, r in enumerate(results):
        ...
        if update.get("status_changed") and update.get("status"):
            ...
            # No status change, just mark as checked
            updates["lastSeen"] = today
            doc_ref.update(updates)
...
def run_lifecycle_search(db):
    ...
    if loop and loop.is_running():
        try:
            ...
            return {"updated": 0, "checked": 0, "errors": 0}
        return loop.run_until_complete(run_lifecycle_monitoring(db))
    else:
        return asyncio.run(run_lifecycle_monitoring(db))
