...
Pulls latest data from Firestore, generates the briefing via Claude Sonnet,
and stores it back in Firestore under dashboard_state/latest_briefing and
weekly_briefings collection.
"""
...
def _get_latest_newsletter():
    """Fetch the most recent newsletter from Firestore."""
    try:
        doc = db.collection('newsletters').document('latest').get()
        if doc.exists:
            return doc.to_dict()
...
async def main():
    ...
    # 1. Gather data
    print("\n[1/4] Gathering data from Firestore...")
    ...
    project_trends = compute_project_trends(db)
    ...
    indicator_trends = compute_indicator_trends(db)
    ...
    print("\n[3/4] Storing briefing in Firestore...")
    await store_and_distribute_briefing(db, briefing_result)
    print("  Stored in weekly_briefings/ and dashboard_state/latest_briefing")
