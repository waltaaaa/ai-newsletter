...
def _get_briefing(db, week_date=None):
    """Fetch briefing content from Firestore.
    ...
    """
    if week_date:
        docs = db.collection("weekly_briefings").where(
            "date", "==", week_date
        ).limit(1).stream()
        for doc in docs:
            return doc.to_dict()
...
def export_briefing_pdf(db, week_date=None):
    ...
    """
    briefing = _get_briefing(db, week_date)
...
def export_briefing_docx(db, week_date=None):
...
def export_and_upload(db, storage_bucket=None):
    """Generate PDF + DOCX and upload to Firebase Storage.

    Stores download URLs in dashboard_state/latest_briefing for frontend.

    Args:
        db: Firestore client
        storage_bucket: firebase_admin.storage bucket (optional; uses default)

    Returns: dict with pdf_url and docx_url, or empty dict on failure
    """
    from firebase_admin import storage

    bucket = storage_bucket or storage.bucket()
    urls = {}
    briefing = _get_briefing(db)
    if not briefing:
    ...
    # PDF
    try:
        pdf_bytes = export_briefing_pdf(db)
        if pdf_bytes:
            blob = bucket.blob(f"briefings/CAN_Macro_Briefing_{date_str}.pdf")
            blob.upload_from_string(pdf_bytes, content_type="application/pdf")
            blob.make_public()
            urls["pdf_url"] = blob.public_url
            logger.info(f"PDF uploaded: {blob.public_url}")
    except Exception as e:
        logger.warning(f"PDF export failed: {e}")

    # DOCX
    try:
        docx_bytes = export_briefing_docx(db)
        if docx_bytes:
            blob = bucket.blob(f"briefings/CAN_Macro_Briefing_{date_str}.docx")
            blob.upload_from_string(
                docx_bytes,
                content_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            )
            blob.make_public()
            urls["docx_url"] = blob.public_url
            logger.info(f"DOCX uploaded: {blob.public_url}")
    except Exception as e:
    ...
    if urls:
        try:
            db.collection("dashboard_state").document("latest_briefing").update(urls)
            logger.info(f"Briefing download URLs stored: {list(urls.keys())}")
        except Exception as e:
            logger.warning(f"Failed to store download URLs: {e}")
