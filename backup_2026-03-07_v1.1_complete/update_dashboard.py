...
if not firebase_admin._apps:
    ...
    firebase_admin.initialize_app(cred, {
        'storageBucket': 'can-macro-dashboard.firebasestorage.app',
    })
...
def _archive_indicators_to_history(primary_ind: dict) -> None:
    """Write each indicator value to the indicator_history collection for trend tracking.

    Schema per document: {indicator, province, date, value, unit, source, frequency, backfilled}
    """
    ...
    for field, value in nat_vals.items():
        if not value or value == 'N/A':
            continue
        doc_id = f"{today_str}_{field}_national"
    ...
    for province, prov_data in primary_ind.get('provinces', {}).items():
        for field, value in prov_data.items():
            if field.endswith('_src') or not value or value == 'N/A':
                continue
            doc_id = f"{today_str}_{field}_{province}"
            ref = db.collection('indicator_history').document(doc_id)
            batch.set(ref, {
                'indicator': field,
                'province': province,
    ...
    try:
        ...
        print(f"  [HISTORY] Failed to archive indicators: {e}")
...
def append_to_timeseries(payload: dict, financial_markets: dict, boc_rate: str):
    ...
    for cat in payload.get('commodities', []):
        for item in (cat.get('items', []) if isinstance(cat, dict) else []):
            name = item.get('name', '')
            doc_id = COMM_ID_MAP.get(name)
            if doc_id:
                _upsert(doc_id, name, item.get('unit', ''), 'Commodities',
    ...
    for idx in financial_markets.get('indices', []):
        name = idx.get('name', '')
        doc_id = IDX_ID_MAP.get(name)
        if doc_id:
            _upsert(doc_id, name, 'pts', 'Equity Indices',
                    idx.get('value', '').replace(',', ''))
    ...
    for fx_item in financial_markets.get('fx', []):
        ...
        doc_id = FX_ID_MAP.get(name)
        if doc_id:
            _upsert(doc_id, name, '', 'Foreign Exchange',
                    fx_item.get('value', '').replace(',', ''))
...
def _verify_project_evidence_urls(db, batch_size=200) -> None:
    """
    ...
    try:
        docs = list(db.collection('projects').where(
            'urls_checked_at', '<', cutoff
        ).limit(batch_size).stream())
    except Exception:
        # Field may not exist yet — query all unchecked projects
        try:
            docs = []
            for d in db.collection('projects').limit(batch_size).stream():
                data = d.to_dict()
                if not data.get('urls_checked_at'):
                    docs.append(d)
    ...
    url_tasks = []  # (doc_ref, evidence_index, url)
    for doc in docs:
        data = doc.to_dict()
        for i, ev in enumerate(data.get('evidence', [])):
            url = ev.get('url', '')
            if url and url.startswith('http') and not ev.get('url_dead'):
                url_tasks.append((doc.reference, doc.id, i, url))

    if not url_tasks:
        # Mark all as checked even if no URLs
        for doc in docs:
            try:
                doc.reference.update({'urls_checked_at': date.today().isoformat()})
            except Exception:
                pass
    ...
    # Batch HEAD checks
    urls_only = [t[3] for t in url_tasks]
    with concurrent.futures.ThreadPoolExecutor(max_workers=12) as ex:
    ...
    updates_by_doc = {}
    dead_count = 0
    for (doc_ref, doc_id, ev_idx, url), is_live in zip(url_tasks, results):
        if not is_live:
            dead_count += 1
            if doc_id not in updates_by_doc:
                updates_by_doc[doc_id] = {'ref': doc_ref, 'dead_indices': []}
            updates_by_doc[doc_id]['dead_indices'].append(ev_idx)

    # Apply updates
    checked_docs = set()
    for doc_id, info in updates_by_doc.items():
        try:
            doc_data = info['ref'].get().to_dict()
            evidence = doc_data.get('evidence', [])
            changed = False
            for idx in info['dead_indices']:
                if idx < len(evidence) and not evidence[idx].get('url_dead'):
                    evidence[idx]['url_dead'] = True
                    changed = True
            if changed:
                info['ref'].update({
                    'evidence': evidence,
                    'urls_checked_at': date.today().isoformat(),
                })
            checked_docs.add(doc_id)
        except Exception:
            pass

    # Mark remaining docs as checked
    for doc in docs:
        if doc.id not in checked_docs:
            try:
                doc.reference.update({'urls_checked_at': date.today().isoformat()})
            except Exception:
                pass
...
def _check_stale_projects(db) -> None:
    ...
    try:
        docs = db.collection('projects').where('lastSeen', '<', stale_cutoff).limit(50).stream()
        stale = list(docs)
        if not stale:
            print("  [Stale check] No projects older than 4 weeks.")
            return
        print(f"  [Stale check] Marking {len(stale)} stale projects...")
        for doc in stale:
            p = doc.to_dict()
            name = p.get('name', '')
            if not name:
            ...
            try:
                doc.reference.update({
                    'stale': True,
                    'statusNote': f"Not seen since {p.get('lastSeen', 'unknown')}",
                })
            except Exception:
                pass
        print(f"  [Stale check] {len(stale)} projects marked stale")
    except Exception as e:
...
def seed_projects(deep_sweep: bool = False) -> None:
    ...
    try:
        unbackfilled = db.collection('projects').where(
            'history_backfilled', '==', False
        ).stream()
        bf_count = 0
        for doc in unbackfilled:
            p = doc.to_dict()
            name = p.get('name', '')
            source_url = ''
            for entry in (p.get('statusHistory') or []):
            ...
            if result.get('history_backfilled') and result.get('statusHistory'):
                doc.reference.update({
                    ...
                    'history_earliest_date': result.get('history_earliest_date', ''),
                    'statusHistory': result['statusHistory'] + (p.get('statusHistory') or []),
                })
                bf_count += 1
...
def update_dashboard(deep_sweep: bool = False):
    try:
        ...
        try:
            ...
            pro_follow_ups = get_follow_up_queries(db)
            if pro_follow_ups:
        ...
        if all_flat_projects:
            ...
            upsert_flat_projects(db, verified)
        else:
        ...
        if gemini_projects:
            ...
            for doc in db.collection('projects').stream():
                existing_names.add(doc.to_dict().get('name', '').lower().strip())
        ...
        try:
            ...
            cost_results = run_cost_search(db)
                ...
                if needs_enrichment:
                    ...
                    if enriched:
                        ...
                        if enriched_verified:
                            upsert_flat_projects(db, enriched_verified)
        ...
        try:
            new_projects = db.collection('projects').where(
                'history_backfilled', '==', False
            ).limit(20).stream()
            backfill_count = 0
            for doc in new_projects:
                p = doc.to_dict()
                name = p.get('name', '')
                source_url = ''
                # Find source URL from statusHistory
                for entry in (p.get('statusHistory') or []):
                    src = entry.get('source', {})
                    if src.get('url'):
                        source_url = src['url']
                ...
                if result.get('history_backfilled'):
                    ...
                    # Prepend historical entries before existing statusHistory
                    history = result.get('statusHistory', [])
                    if history:
                        from firebase_admin import firestore as _fs
                        existing_history = p.get('statusHistory', [])
                        full_history = history + existing_history
                        update_data['statusHistory'] = full_history
        ...
        try:
            _verify_project_evidence_urls(db)
        except Exception as e:
        ...
        try:
            ...
            monitor_result = run_lifecycle_search(db)
        except Exception as e:
        ...
        try:
            ...
            cap_result = run_capacity_tiers(db)
        except Exception as e:
        ...
        try:
            ...
            missed_result = process_pending_submissions(db, max_queries=20)
        ...
        try:
            ...
            applied = apply_pending_improvements(db)
            if applied:
        ...
        try:
            ...
            if sweep_by_province:
                ...
                if follow_ups:
                    store_follow_up_queries(db, follow_ups)
            ...
            if rss_failed:
                ...
                if recovered:
                    upsert_flat_projects(db, recovered)
            ...
            if flat_for_dedup and len(flat_for_dedup) > 10:
                ...
                if dedup_flags:
                    store_dedup_results(db, dedup_flags)

            # Task 5: Monthly meta-analysis (first week only)
            if datetime.now().day <= 7:
                print("  [CLAUDE] Running monthly meta-analysis...")
                meta = run_meta_analysis_sync(db)
                if meta:
                    store_meta_analysis(db, meta)
        ...
        try:
            ...
            sector_data = compute_project_trends(db)
            indicator_data = compute_indicator_trends(db)
            xref_data = cross_reference_trends(indicator_data, sector_data)
            trend_report = generate_trend_report(
                sector_data, indicator_data, xref_data, db=db
            )

            # Store trend snapshot in Firestore
            if sector_data and not sector_data.get("error"):
                db.collection("trend_snapshots").document(
        ...
        try:
            ...
            policy_developments = process_policy_feeds(db, since_days=7)
        except Exception as e:
        ...
        try:
            ...
            commodity_data = fetch_and_store_commodities(db)
        except Exception as e:
        ...
        try:
            ...
            upcoming_events = get_and_store_events(db, days_ahead=14)
        except Exception as e:
        ...
        try:
            ...
            try:
                ...
                if topic_context and topic_context.get("topic"):
                    ...
                    if microscope_result:
                        ...
                        store_microscope_history(db, topic_context["topic"], microscope_text)
            ...
            if briefing:
                _aio.run(store_and_distribute_briefing(db, briefing))
                ...
                try:
                    ...
                    export_urls = export_and_upload(db)
                    if export_urls:
        ...
        try:
            ...
            permit_anomalies = detect_permit_anomalies(db)
            if permit_anomalies:
                from pipeline_state import store_follow_up_queries as _store_fq
                _store_fq(db, permit_anomalies)
...
if __name__ == "__main__":
    ...
    if args.test_sentiment:
        ...
        if indicators and db:
            dated_id = date.today().strftime("%Y-%m-%d")
            db.collection('timeseries').document(dated_id).set(indicators, merge=True)
            print(f"[OK] Indicators stored to timeseries/{dated_id}")
        else:
