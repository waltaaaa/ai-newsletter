...
Queries SQLite for projects where value is "Not disclosed" or empty,
then uses Gemini to find capital cost information.
...
def _fetch_projects_needing_values(conn, limit):
...
def _build_query(project):
    ...
    return {
        ...
        "_norm_key": project.get('norm_key', project.get('name', '')),
    }
...
async def backfill(conn, limit, dry_run):
    ...
    projects = _fetch_projects_needing_values(conn, limit)
    ...
    for i, result in enumerate(results):
        ...
        # Update SQLite
        try:
            # Find the project dict and update it
            project = next((p for p in projects if p.get('norm_key', p.get('name', '')) == norm_key), None)
            if project:
                updated_project = dict(project)
                updated_project['value'] = value
                updated_project['value_millions'] = value_millions
                updated_project['_value_source'] = 'gemini_backfill'
                updated_project['_value_updated'] = datetime.now(timezone.utc).isoformat()
                if parsed.get('source_url'):
                    updated_project['_value_source_url'] = parsed['source_url']
                upsert_project(conn, updated_project)
                updated += 1
        except Exception as e:
            logger.error(f"  SQLite update failed for {norm_key}: {e}")
            failed += 1
...
def main():
    ...
    parser.add_argument('--dry-run', action='store_true',
                        help='Show what would be updated without writing to SQLite')
    args = parser.parse_args()

    conn = init_db()
    try:
        asyncio.run(backfill(conn, args.limit, args.dry_run))
    finally:
        conn.close()
