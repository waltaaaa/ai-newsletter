...
- **Tavily:** Targeted enrichment searches only (cost-finding, verification, named tracking). Free tier 1,000 credits/month. (One-time exception: user approved up to 2,000 credits for 2026-06 only, for the `tools/tavily_cost_batch.py` cost-finding batch. The standing budget remains 1,000/month — the weekly pipeline's `tavily_search.py` cap is unchanged.)
...
Extraction queue (recall-fix 2026-06-10): Phase 3 extraction consumes the remediated 6-layer output (`rss_filtered`, stamped `_l6_passed`) — NOT a raw keyword re-cut of Phase 1 items. Articles are extracted highest rerank relevance first (`_rerank_logit` stamped in L7), so a phase timeout only ever costs the low-signal tail. The unprocessed remainder persists to `dashboard_state:extraction_backlog` and is carried over at top priority next run (3-attempt cap).
...
- **Edition archive is APPEND-ONLY.** `docs/data/briefing_archive.json` (the previous-editions dropdown) must never lose an edition — on 2026-06-08 a wholesale rebuild collapsed it from 7 entries to 1. Three defenses: `export_briefings` union-merges with the on-disk file and refuses to shrink it (preserving `file_date`, which points at the actual dated `briefing_<file_date>.json` when it differs from `week_of`); `tools/validate_briefing_schema.py` FAILs the deploy gate if any HEAD-published edition is missing, if an entry's dated file doesn't exist (dropdown 404), or if the archive is empty; `tools/_rebuild_briefing_archive.py` preserves non-re-derivable entries and requires `--force` to shrink. Every archive entry's dated briefing file must ship with it.
- **Event-sourced discipline (quality-pass-1.4).** Material project mutations append events rather than mutating silently: status changes write `project_changes`/`project_events`, evidence additions write `evidence` rows, and confidence decay writes one `confidence_decay` event per project per run (old→new, decay bucket). New code that mutates project state SHOULD log an event row alongside the UPDATE. (Full projection-rebuild from events is NOT claimed — the events are an audit trail, not the source of truth.)
...
- `projects` — main project database. Value-semantics columns (quality-pass-1.4, additive): `currency` (default 'CAD'), `value_low`/`value_high` (millions, only when a range is stated — never fabricated), `value_scope` ('phase'|'program'|''), plus `axes_satisfied` (count of distinct triangulation axes with ≥1 observation — see `triangulation.py`). Merge precedence: a government/registry-backed value beats a media-sourced value regardless of recency.
- `evidence` — normalized evidence rows. `content_hash` (sha256 of normalized title+snippet) is populated at insert; same-project hash collisions set `republication_of` → original rowid. Republications carry ZERO confidence weight in both scorers; their URLs are always preserved.
...
8 additional StatCan WDS tables fetched in Phase 1 (data collection) covering capital expenditure intentions (34-10-0035, annual), building investment (34-10-0175, quarterly), construction price index (18-10-0135, quarterly), employment by industry (14-10-0022, monthly), job vacancies (14-10-0326, quarterly), merchandise exports (12-10-0163, monthly), housing starts (34-10-0143, monthly), and new housing price index (18-10-0205, monthly). All fetched via WDS API. Zero cost, no API key. Mode-aware: daily/indicators-only runs skip annual and quarterly tables, only fetch monthly ones. Indicators feed into `indicator_history` and the cross-reference engine.

Plus metadata-resolved series (2026-06-11, `META_RESOLVED_GROUPS` in `statcan_extended.py`): 11 LFS sector-employment series (14-10-0022), national average hourly wage (14-10-0063), job vacancies monthly SA (14-10-0372), manufacturing/retail/wholesale sales (16-10-0047, 20-10-0008, 20-10-0074), national building permits res/non-res (34-10-0292), and household disposable income + saving rate (36-10-0112, quarterly). These resolve coordinates AT RUNTIME from cube metadata by dimension-member NAME (no hardcoded vectors), and every observation must pass a per-series plausibility range AND a freshness gate (`_is_fresh`: monthly ≤120d, quarterly ≤300d, annual ≤730d) before saving — a renamed member, archived cube, or wrong match skips loudly instead of writing stale/wrong data (the agri-exports-frozen-at-2003 failure mode). The freshness gate also applies to all hardcoded-vector groups. indicator_name keys match what the frontend `IND_KEY_INDICATORS` tables reference. Legacy frontend keys (`nat_unemployment`, `nat_participation_rate`, `nat_employment_rate`, `cpi_national`, `boc_rate`) are translated to canonical export names by `_IND_KEY_ALIASES` in `app.js` — the export intentionally publishes those series as indicator+province='national'. Phase 1 archives (`_archive_indicators_to_history`) stamp the StatCan REFERENCE period, never the fetch date, and carry previous_value/change into `indicator_history`.
