"""
seed_projects.py — Perplexity Sonar Pro + Claude Haiku 4.5 Project Seeder

Two-stage pipeline:
  1. Perplexity Sonar Pro  — web search, NAICS-targeted queries per province
  2. Claude Haiku 4.5      — parse and structure results into project schema
...
# ==========================================
# PROJECT SCHEMA (passed to Claude Haiku)
# ==========================================
...
# ==========================================
# CLAUDE HAIKU STRUCTURED PARSING
# ==========================================

def parse_with_haiku(raw_text: str, province: str, threshold: str, naics_code: str, citations: list[str] | None = None) -> list:
    """Use Claude Haiku 4.5 to structure Perplexity's raw output into project records."""
...
{raw_text}"""
    ...
    for attempt in range(4):
        try:
            ...
            if attempt == 3:
                print(f"\n    [HAIKU JSON ERROR] {province}/{naics_code}: {e}")
                return []
            time.sleep(1)
        except Exception as e:
            if attempt == 3:
                print(f"\n    [HAIKU ERROR] {province}/{naics_code}: {e}")
                return []
            time.sleep(2 ** attempt)
    return []
...
def seed_projects():
    ...
    for prov_cfg in PROVINCE_CONFIG:
        ...
        for naics_code, sector_label in naics_list:
            ...
            # Step 2: Claude Haiku structured parsing
            projects = parse_with_haiku(raw_text, prov_name, threshold, naics_code, citations)
    ...
    print(f"  Raw from Haiku:     {total_raw}")
...
def seed_active_legacy(existing_docs: dict, existing_names_by_province: dict) -> int:
    ...
    for prov_cfg in LEGACY_CONFIG:
        ...
        for i, template_fn in enumerate(ACTIVE_QUERY_TEMPLATES, 1):
            ...
            # Reuse parse_with_haiku — "mixed" naics_code signals multi-sector context
            projects = parse_with_haiku(raw_text, prov_name, threshold, "mixed", citations)
    ...
    print(f"{'=' * 55}")
    print(f"  Perplexity calls:   {perplexity_calls}")
    print(f"  Raw from Haiku:     {raw_total}")
