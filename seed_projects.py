...
Deduplicates against SQLite and within the current run.
...
def seed_projects(conn):
    ...
    print("Loading existing projects from SQLite...")
    existing_projects = get_all_projects(conn)
    existing_docs = {}
    existing_names_by_province = {}
    for data in existing_projects:
        key = normalize_key(data['name'], data['province'])
        existing_docs[key] = data
    ...
    for prov_cfg in PROVINCE_CONFIG:
        ...
        for naics_code, sector_label in naics_list:
            ...
            for project in projects:
                ...
                existing_docs[exact_key] = new_doc
    ...
    print(f"  Added to SQLite:    {total_added}")
...
def seed_active_legacy(conn, existing_docs: dict, existing_names_by_province: dict) -> int:
    """
    ...
    print(f"  Added to SQLite:    {added_total}")
    print(f"  Skipped (dupes):    {skipped_total}")
    print(f"{'=' * 55}\n")
...
if __name__ == "__main__":
    ...
    existing_docs, existing_names_by_province = seed_projects(conn)
    seed_active_legacy(conn, existing_docs, existing_names_by_province)
    conn.close()
